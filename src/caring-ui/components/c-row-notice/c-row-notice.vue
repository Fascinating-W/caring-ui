<template>
	...
	>
		<view class="c-direction-row">
			...
			<view class="c-notice-box" id="c-notice-box">
				<view
					class="c-notice-content"
					id="c-notice-content"
					:style="{
				...
				>
					<text class="c-notice-text" @tap="click" :style="[textStyle]"
					:class="['c-' + type]">{{showText}}</text>
				</view>
			</view>
			<view class="c-icon-wrap">
	...
	methods: {
		initSize() {
			...
			Promise.all(query).then(() => {
				// 根据t=s/v(时间=路程/速度)，这里为何不需要加上#c-notice-box的宽度，因为中设置了.c-notice-content样式中设置了padding-left: 100%
				// 恰巧计算出来的结果中已经包含了#c-notice-box的宽度
