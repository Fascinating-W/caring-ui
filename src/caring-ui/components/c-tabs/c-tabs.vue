<template>
  <view
    ...
    :class="customClass"
  >
    <!-- $c.getRect()对组件根节点无效，因为写了.in(this)，故这里获取内层接点尺寸 -->
    <view>
      <scroll-view
        scroll-x
        class="c-scroll-view"
      ...
      >
        ...
        >
          ...
          <view v-if="showBar" class="c-tab-bar" :style="[tabBarStyle]"></view>
        </view>
      </scroll-view>
    </view>
  </view>
...
export default {
    ...
    tabItemStyle() {
      return (index) => {
        let style = {
          ...
          padding: this.scroll ? `0 ${this.gutter}px` : '',
          flex: this.scroll ? 'auto' : '1',
          width: this.$c.addUnit(this.itemWidth)
        }
        // 字体加粗
  ...
  methods: {
    ...
    async init() {
      // 获取tabs组件的尺寸信息
      let tabRect = await this.$cGetRect('#' + this.id)
      // tabs组件距离屏幕左边的宽度
    ...
    getTabRect() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        // 只要size和rect两个参数
        query.select(`#c-tab-item-${i}`).fields({
          size: true,
          rect: true
        })
      }
