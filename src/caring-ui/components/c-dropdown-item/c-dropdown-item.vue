<template>
	<view class="c-dropdown-item" v-if="active" @touchmove.stop.prevent="() => {}" @tap.stop.prevent="() => {}">
	...
	export default {
		...
		data() {
			return {
				...
				activeColor: uni.$c.color.primary, // 激活时左边文字和右边对勾图标的颜色
				inactiveColor: uni.$c.color.content, // 未激活时左边文字和右边对勾图标的颜色
			}
		},
		computed: {
			// 监听props是否发生了变化，有些值需要传递给父组件c-dropdown，无法双向绑定
			propsChange() {
		...
		methods: {
			init() {
				// 获取父组件c-dropdown
				let parent = this.$c.$parent.call(this, 'c-dropdown');
				if (parent) {
					this.parent = parent;
