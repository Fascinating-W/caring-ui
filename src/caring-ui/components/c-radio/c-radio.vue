<template>
  <view class="c-radio" :style="[radioStyle]">
    <view
...
export default {
  ...
  computed: {
    ...
    iconStyle() {
      ...
      style.width = this.$c.addUnit(this.elSize)
      style.height = this.$c.addUnit(this.elSize)
      return style
    },
    iconColor() {
    ...
    radioStyle() {
      ...
      if (this.parentData.width) {
        style.width = this.$c.addUnit(this.parentData.width)
        // #ifdef MP
        ...
        // #ifndef MP
        // H5和APP使用flex布局
        style.flex = `0 0 ${this.$c.addUnit(this.parentData.width)}`
        // #endif
      }
  ...
  methods: {
    updateParentData() {
      this.getParentData('c-radio-group')
    },
    onClickLabel() {
