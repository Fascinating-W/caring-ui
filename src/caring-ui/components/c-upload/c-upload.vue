<template>
	<view class="c-upload" v-if="!disabled">
		<view
			v-if="showUploadList"
			class="c-list-item c-preview-wrap"
			v-for="(item, index) in lists"
			:key="index"
			:style="{
				width: $c.addUnit(width),
		...
		>
			...
			>
				<c-icon class="c-icon" :name="delIcon" size="10" :color="delColor"></c-icon>
			</view>
			<c-line-progress
			...
			></c-line-progress>
			<view @tap.stop="retry(index)" v-if="item.error" class="c-error-btn">点击重试</view>
			<image @tap.stop="doPreviewImage(item.url || item.path, index)" class="c-preview-image" v-if="!item.isImage" :src="item.url || item.path" :mode="imageMode"></image>
		</view>
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			<view
				v-if="!customBtn"
				class="c-list-item c-add-wrap"
				hover-class="c-add-wrap__hover"
				hover-stay-time="150"
				:style="{
					width: $c.addUnit(width),
					height: $c.addUnit(height)
			...
			>
				<c-icon name="plus" class="c-add-btn" size="20"></c-icon>
				<view class="c-add-tips">{{ uploadText }}</view>
			</view>
		</view>
	</view>
</template>
	...
	methods: {
		...
		async uploadFile(index = 0) {
			...
			if(this.beforeUpload && typeof(this.beforeUpload) === 'function') {
				...
				// 因为upload组件可能会被嵌套在其他组件内，比如c-form，这时this.$parent其实为c-form的this，
				// 非页面的this，所以这里需要往上历遍，一直寻找到最顶端的$parent，这里用了this.$c.$parent.call(this)
				// 明白意思即可，无需纠结this.$c.$parent.call(this)的细节
				let beforeResponse = this.beforeUpload.bind(this.$c.$parent.call(this))(index, this.lists);
				// 判断是否返回了promise
				...
				success: async (res) => {
					if (res.confirm) {
						...
						if(this.beforeRemove && typeof(this.beforeRemove) === 'function') {
							...
							let beforeResponse = this.beforeRemove.bind(this.$c.$parent.call(this))(index, this.lists);
							// 判断是否返回了promise
							if (!!beforeResponse && typeof beforeResponse.then === 'function') {
								await beforeResponse.then(res => {
									// promise返回成功，不进行动作，继续上传
