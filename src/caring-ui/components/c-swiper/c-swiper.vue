<template>
  ...
  >
    <swiper
      ...
      :next-margin="effect3d ? effect3dPreviousMargin + 'px' : '0'"
      :style="{
        height: height + 'px',
    ...
    >
      ...
      >
        <view
          class="c-list-image-wrap"
          @tap.stop.prevent="listClick(index)"
          :class="[uCurrent != index ? 'c-list-scale' : '']"
          :style="{
            borderRadius: `${rounded}px`,
            transform:
    ...
    >
      <block v-if="mode == 'rect'">
        <view
          class="c-indicator-item-rect"
          :class="{ 'c-indicator-item-rect-active': index == uCurrent }"
          v-for="(item, index) in list"
          :key="index"
        ></view>
      </block>
      <block v-if="mode == 'dot'">
        <view
          class="c-indicator-item-dot"
          :class="{ 'c-indicator-item-dot-active': index == uCurrent }"
          v-for="(item, index) in list"
          :key="index"
        ></view>
      </block>
      <block v-if="mode == 'round'">
        <view
          class="c-indicator-item-round"
          :class="{ 'c-indicator-item-round-active': index == uCurrent }"
          v-for="(item, index) in list"
          :key="index"
        ></view>
      </block>
      <block v-if="mode == 'number'">
        <view class="c-indicator-item-number"
...
export default {
  ...
  props: {
    ...
    // 3D模式的情况下，激活item与前后item之间的距离，单位px
...
.c-indicator-item-rect-active {
  background-color: rgba(255, 255, 255, 0.8);
}

.c-indicator-item-dot {
  width: 14rpx;
  height: 14rpx;
  margin: 0 6rpx;
  border-radius: 20rpx;
...
.c-indicator-item-dot-active {
  background-color: rgba(255, 255, 255, 0.8);
}

.c-indicator-item-round {
  width: 14rpx;
  height: 14rpx;
  margin: 0 6rpx;
  border-radius: 20rpx;
...
.c-indicator-item-round-active {
  width: 34rpx;
  background-color: rgba(255, 255, 255, 0.8);
}

.c-indicator-item-number {
  padding: 6rpx 16rpx;
  line-height: 1;
  background-color: rgba(0, 0, 0, 0.3);
